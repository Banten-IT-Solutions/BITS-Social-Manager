...
accountsRouter.get('/', async (c) => {
  ...
  if (projectId) {
    ...
    const accounts = await db.select().from(socialAccounts).where(eq(socialAccounts.projectId, projectId)).all();
  ...
  const userProjects = await db.select({ id: projects.id }).from(projects).where(eq(projects.userId, userId)).all();
...
accountsRouter.post('/', zValidator('json', createSchema), async (c) => {
  ...
  await db.insert(socialAccounts).values({ id, projectId, platform, accountName, emailHandle, passwordEncrypted, notes }).run();
